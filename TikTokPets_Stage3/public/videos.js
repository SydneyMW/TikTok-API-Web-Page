...
// Add the blockquote element that tiktok will load the video
async function addVideo(tiktokurl,divElmt,idNum,nickname) {
  let videoNumber = tiktokurl.split("video/")[1];
  divElmt.nextSibling.nextSibling.textContent = nickname;
  let block = document.createElement('blockquote');
  block.className = "tiktok-embed";
  block.cite = tiktokurl;
  // have to be formal for attribute with dashes
  block.setAttribute("data-video-id",videoNumber);
  block.setAttribute("rowIdNum",idNum);
